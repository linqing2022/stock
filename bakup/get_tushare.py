...
mkdir('.\\tushare\\hfq_fina\\')
mkdir('.\\tushare\\daily_basic')
...
for i in range(len(data)) :
    ...
    if (df_hfq is not None and df_fina is not None and df_daily_basic is not None):
        ...
        df_hfq.to_csv('.\\tushare\\hfq\\' + code + '.csv',index=False)
        df_fina = df_fina.sort_values('end_date')
        df_fina.to_csv('.\\tushare\\fina\\' + code + '.csv',index=False)
        df_daily_basic = df_daily_basic.sort_values('trade_date')
        df_daily_basic.to_csv('.\\tushare\\daily_basic\\' + code + '.csv',index=False)
        note.write(code+'\n')
note.close()
