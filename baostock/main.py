...
def stock_trade(stock_file):
    ...
    for loop in range(100):
        ...
        model = PPO2(MlpPolicy, env, verbose=0, tensorboard_log=Path('./log').name)
        # model.load("PPO2")
...
def test_a_stock_trade(stock_code):
    stock_file = find_file('./baostock/train', str(stock_code))
...
def multi_stock_trade():
    ...
    for code in range(start_code, start_code + max_num):
        stock_file = find_file('./baostock/train', str(code))
...
if __name__ == '__main__':
    # multi_stock_trade()
    test_a_stock_trade('002466.SZ')
    # test_a_stock_trade('sz.002466')
    # ret = find_file('./baostock/train', '600036')
    # print(ret)
